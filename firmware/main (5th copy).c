...
void VO_Iterate(voice_t *vo){
	...
	// compute the signal
	//s = ((vol * s) / 256);
	vo->signal = s; //(s * (uint8_t)REG(MASTER_GAIN)); //CLAMP((out * REG(MASTER_GAIN)) / 2, 0, 255); //vo->osc[1].output * REG(MASTER_GAIN); 
	static uint16_t p0 = 0, p1 = 0, p2 = 0, p3 = 0, fm = 0; 
	static uint16_t w0 = INCREMENT_FROM_FREQ(220) ;
	static uint16_t w1 = INCREMENT_FROM_FREQ(440) ;
	static uint16_t w2 = INCREMENT_FROM_FREQ(880) ;
	static uint16_t w3 = INCREMENT_FROM_FREQ(1760) ;
	p0 += w0;
	p1 += w1;
	p2 += w2;
	p3 += w3; 
	OCR0A = (SAWL(p0 >> 8) + SAWR(p1 >> 8) + SQUARE(p2 >> 8) + TRIANGLE(p3 >> 8)) >> 2;
	//(SAWL(phase / 256) + SIN((SIN(fm / 256) + phase) & 0xff) / 4) + 128; 
...
int main(void){
	...
	REG(OSC2_FREQ_H) = 3;
	REG(OSC2_FREQ_L) = 0x70;
	REG(OSC2_PHASE_OFFSET) = 0;
	REG(OSC2_VOLUME) = 255;
	REG(OSC2_DETUNE_COARSE) = 24;
	REG(OSC2_DETUNE_FINE) = 0;
	REG(OSC2_WAVEFORM) = WAVE_SINE; 
